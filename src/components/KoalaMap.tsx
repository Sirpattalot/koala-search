...
const KoalaMap = ({
  ...
  const generateKoalaTextbox = (
    probability: number,
    coords: LatLng
  ): string => {
    const probabilityText = getProbabilityText(probability);
    const coordString = coords.lat + ',' + coords.lng;
    const reportSightingUrl =
      'https://www.environment.nsw.gov.au/topics/animals-and-plants/threatened-species/programs-legislation-and-framework/nsw-koala-strategy/how-to-get-involved/privacy-policy';

    return `It is <b>${probabilityText}</b> you'll find me within 5km of here.<br><br>
  ...
  const generateBaseMap = (): void => {
    ...
    let grayscale = L.tileLayer(mapboxUrl, {
      ...
      tileSize: 512,
      zoomOffset: -1,
      attribution: 'powered by MapBox & Leaflet',
    });
  ...
  const redrawKoalaLayer = (targetLocation: LatLngLiteral): void => {
    if (!layerRef?.current) {
      return;
    }

    layerRef.current.clearLayers();
    const filteredKoalaData = getKoalasNearby(
      targetLocation,
      maxDistance,
      maxResults,
      minProbability
  ...
  const getKoalasNearby = (
    ...
    return (
      Object.assign({}, koalaData)
        .features.sort((a: GeoJSON.Feature, b: GeoJSON.Feature) => {
          //having trouble typing these.
          let geometryA: any = a.geometry;
          let geometryB: any = b.geometry;

          const distanceA = getCrowDistanceBetween(
            location.lat,
            location.lng,
            geometryA.coordinates[1],
            geometryA.coordinates[0]
          );
          const distanceB = getCrowDistanceBetween(
            location.lat,
            location.lng,
            geometryB.coordinates[1],
            geometryB.coordinates[0]
          );
        ...
        .filter((feature: GeoJSON.Feature) => {
          if (feature?.properties?.p > minProbability) {
            ...
            const distance = getCrowDistanceBetween(
              location.lat,
              location.lng,
  ...
  const moveMap = (koalaCount: number, latLngs: LatLng[]): void => {
    if (!leafletMap) {
      return;
    }

    if (koalaCount > 0) {
      //Contain all search result icons in map view.
      const bounds: LatLngBoundsExpression = L.latLngBounds(latLngs);
      leafletMap.flyToBounds(bounds);
    } else if (location) {
      //else just go to selected location - no koalas :(
      leafletMap.flyTo([location.lat, location.lng], 9);
    }
  };

  const getGeoJSONLayer = (features: GeoJSON.Feature[]) => {
    const filteredKoalaData: GeoJSON.FeatureCollection = {
    ...
    let koalaLayer = L.geoJSON(filteredKoalaData, {
      pointToLayer: (
        geoJsonPoint: GeoJSON.Feature<GeoJSON.Point, any>,
        latLng: LatLng
      ) => {
        latLngs.push(latLng);
  ...
  return (
    <>
      <KoalaCountNotification koalaResultCount={koalaResultCount} />
