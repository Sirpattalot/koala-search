...
function FilterPanel({ onChangeProbablity, onChangeResultCount, onChangeDistance, minProbability, maxDistance, maxResults }: FilterPanelProps) {

  const handleDistanceFilterChange = (event: Event, newValue: number | Number[]) => {
    const distance = newValue as number;
    onChangeDistance(distance);
  };

  const handleResultsFilterChange = (event: Event, newValue: number | Number[]) => {
    const resultCount = newValue as number;
    onChangeResultCount(resultCount);
  };

  const handleProbabilityFilterChange = (event: Event, newValue: number | Number[]) => {
